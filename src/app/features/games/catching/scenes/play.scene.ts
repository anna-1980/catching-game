...
export class PlayScene extends Phaser.Scene {
  ...
  create() {
    ...
    //set up overlap detection after both exist
        this.physics.add.overlap(this.bucket, this.balls, (bucket, ball) => {
          this.catchBall(
            bucket as Phaser.Physics.Arcade.Sprite,
            ball as Phaser.GameObjects.Arc
          );
        });
  ...
  private spawnBall() {
    ...
    const ball = this.add.circle(x, 0, 10, type.color);
  ...
  /// ---- detect catches---------------
  private catchBall(
    bucket: Phaser.Physics.Arcade.Sprite,
  ...
  private gameOver() {
    ...
    restartText.setText('Press [b]R[/b] to restart');
    menuText.setText('Press [b]M[/b] to return to main menu');
